...
function WorldModelView() {
  ...
  const run = async () => { const running = !Boolean(status?.running); setMessage(running ? 'Starting simulated Body loop...' : 'Pausing simulated Body loop...'); try { await checked(await fetch('/api/interface/body/worldmodel/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ running }) })); await read(); } catch (e) { setMessage(e instanceof Error ? e.message : 'World model run control failed.'); } };
