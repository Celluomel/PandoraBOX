...
function Fnk0031Activity() {
  ...
  return <section className="fnk-activity" aria-label="FNK0031 locomotion controller visualization"><div className="fnk-activity-head"><div><span className="settings-section-title">Locomotion controller</span><p className="muted">Local visual simulation · CPG + Izhikevich + R-STDP</p></div><span className="fnk-live"><i/>RUNNING</span></div><div className="fnk-activity-body"><svg viewBox="0 0 240 120" role="img" aria-label="Animated six leg tripod gait"><defs><radialGradient id="fnk-core"><stop stopColor="#9ff2c5" stopOpacity=".55"/><stop offset="1" stopColor="#173b2b" stopOpacity="0"/></radialGradient></defs><ellipse cx="120" cy="60" rx="55" ry="38" fill="url(#fnk-core)"/><rect x="88" y="38" width="64" height="44" rx="18" fill="#152b22" stroke="#7ed9ab" strokeWidth="1.5"/><text x="120" y="63" textAnchor="middle" fill="#c8f5d9" fontSize="9" fontFamily="monospace">FNK0031</text>{legs.map((leg, index) => <g key={index}><line x1={leg.x} y1={leg.y} x2={leg.endX} y2={leg.endY} stroke={index % 2 === 0 ? '#73e0ad' : '#d1a9f4'} strokeWidth="4" strokeLinecap="round" opacity={0.55 + Math.abs(leg.swing) * 0.45}/><circle cx={leg.endX} cy={leg.endY} r="4" fill={index % 2 === 0 ? '#b8f6d0' : '#d8b8ff'}/></g>)}{spikes.map((active, index) => <circle key={index} cx={25 + (index % 9) * 24} cy={104 + (index > 8 ? 8 : 0)} r={active ? 3 : 1.3} fill={active ? '#f2c879' : '#496c5b'} opacity={active ? 1 : .7}/>)}</svg><div className="fnk-metrics"><div><span>CPG phase</span><b>{((phase / (Math.PI * 2)) * 100).toFixed(0)}%</b></div><div><span>SNN spikes</span><b>{spikes.filter(Boolean).length}/18</b></div><div><span>Competence</span><b>{(competence * 100).toFixed(1)}%</b></div><div><span>Gait</span><b>TRIPOD</b></div></div></div><p className="muted fnk-activity-note">The animation demonstrates the controller locally; physical movement requires the FNK0031 Body plugin, ESP32 bridge and explicit actuation approval.</p></section>;
}
